...
public class EditarTimetable extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        timePicker.setCurrentHour(Integer.parseInt(hr.substring(0, 2)));
        timePicker.setCurrentMinute(Integer.parseInt(hr.substring(3, 5)));
        ...
        switch(v.getId()) {
            case R.id.savebutton:
                ...
                if(professor.getText().toString().length() == 0 || materia.getText().toString().length() == 0) {
                    ...
                    Timetable timetable = new Timetable(id, horario, professor.getText().toString(), materia.getText().toString(), diaDaSemana);
                    db.updateMateria(timetable);
